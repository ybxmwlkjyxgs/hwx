...
do
    ...
    ---
    -- @noSelf
    validator.validator_45939911 = function(ctrl, mouse_btn_id)
        local 开关状态 = base.game.object_restore_value(ctrl, "Key")
        if 开关状态 == nil then
            开关状态 = true
        else
        end
        if 开关状态 == true then
            base.game.object_store_value(ctrl, "Key", false)
            ctrl.text = "开启命中特效"
            base.set_actor_creation_filter_level(1)
        else
            base.game.object_store_value(ctrl, "Key", true)
            ctrl.text = "关闭命中特效"
