module validator {
	...
	/////效果开关.lua
	/** @noSelf */
	export let validator_45939911:(this: void, ctrl:_OBJ_gameui_gui_ctrl_normal_btn, mouse_btn_id:number)=>void = (this: void, ctrl:_OBJ_gameui_gui_ctrl_normal_btn, mouse_btn_id:number):void => {
		let 开关状态: boolean | undefined = ((base.game) as unknown as base.Game).object_restore_value(ctrl, "Key");
		if (((开关状态 == undefined))) {
			开关状态 = true;
		} else {
		}
		if (((开关状态 == true))) {
			((base.game) as unknown as base.Game).object_store_value(ctrl, "Key", false);
			ctrl.text = "开启命中特效";
			base.set_actor_creation_filter_level(1);
		} else {
			((base.game) as unknown as base.Game).object_store_value(ctrl, "Key", true);
			ctrl.text = "关闭命中特效";
			base.set_actor_creation_filter_level(0);
		}
	}
